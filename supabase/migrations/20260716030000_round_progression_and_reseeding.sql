          ...
          and (
            not exists (
              ...
              where configured_round.value ->> 'id' = v_round.format_round_id
                and configured_round.value -> 'winCondition' ->> 'type' = 'checkmate'
            )
            or public.checkmate_decisive_game(
              scores.round_id::text,
              coalesce((
                select (configured_round.value -> 'winCondition' ->> 'threshold')::integer
                from jsonb_array_elements(v_tournament.format_config -> 'rounds') configured_round(value)
                where configured_round.value ->> 'id' = v_round.format_round_id
                  and configured_round.value -> 'winCondition' ->> 'type' = 'checkmate'
                limit 1
            ...
            or lobbies.game_number <= public.checkmate_decisive_game(
              scores.round_id::text,
              coalesce((
                select (configured_round.value -> 'winCondition' ->> 'threshold')::integer
                from jsonb_array_elements(v_tournament.format_config -> 'rounds') configured_round(value)
